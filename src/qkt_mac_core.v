// qkt_mac_core.v
// Stateless command-decode 1x4 INT8 dot-product MAC + INT32 accumulator.
// No FSM. Commands decode per cycle.
...
module qkt_mac_core (
    ...
    // ---- Q shift-register (persists across many K vectors) ----
    reg [31:0] q_shift;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)         q_shift <= 32'd0;
        else if (is_load_q) q_shift <= {q_shift[23:0], data_in};
    end

    // ---- K shift-register + byte counter ----
    reg [31:0] k_shift;
    reg [1:0]  k_cnt;
    reg        fire;   // 1-cycle pulse: pipeline start
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            k_shift <= 32'd0; k_cnt <= 2'd0; fire <= 1'b0;
        end else if (is_load_k) begin
            k_shift <= {k_shift[23:0], data_in};
            k_cnt   <= k_cnt + 2'd1;      // wraps 3->0
            fire    <= (k_cnt == 2'd3);   // pulses on the 4th byte
        end else begin
            fire <= 1'b0;
        end
    end

    // ---- Stage 1: 4 signed multiplies ----
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            s1_v <= fire;
            if (fire) begin
                m0 <= $signed(q_shift[31:24]) * $signed(k_shift[31:24]);
                m1 <= $signed(q_shift[23:16]) * $signed(k_shift[23:16]);
                m2 <= $signed(q_shift[15: 8]) * $signed(k_shift[15: 8]);
                m3 <= $signed(q_shift[ 7: 0]) * $signed(k_shift[ 7: 0]);
            end
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin sum <= 0; s2_v <= 1'b0; end
            ...
            s2_v <= s1_v;
            if (s1_v)
                sum <= $signed({{2{m0[15]}}, m0}) + $signed({{2{m1[15]}}, m1}) +
                       $signed({{2{m2[15]}}, m2}) + $signed({{2{m3[15]}}, m3});
        end
    end
    ...
    // ---- Read-out: on READ_ACC, stream 4 bytes MSB-first ----
    // Cycle after cmd: data_out = acc[31:24], read_cnt=1
    // Next cycles:     acc[23:16], acc[15:8], acc[7:0]
    reg [2:0] read_cnt;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
